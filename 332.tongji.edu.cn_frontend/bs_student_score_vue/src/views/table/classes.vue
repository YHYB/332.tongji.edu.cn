  ...
  <div>
    ...
    <el-form :inline="true">
      <el-form-item label="班级名称">
        <el-input placeholder="班级名称" size="small" v-model="searchMap.class_name"></el-input>
      </el-form-item>
      <el-form-item label="班主任">
        <el-input placeholder="班主任" size="small" v-model="searchMap.teacher"></el-input>
      </el-form-item>
    ...
    <el-table
      ...
      <el-table-column label="班主任" prop="teacher"></el-table-column>
      <el-table-column label="工号" prop="job_num"></el-table-column>
      <el-table-column label="联系方式" prop="contact_info"></el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="编辑" width="40%">
      <el-form label-width="100px" style="margin-right: 30px">
        <el-form-item label="班级名称">
          <el-input size="medium" v-model="pojo.class_name"></el-input>
        </el-form-item>
        <el-form-item label="班主任">
          <el-input size="medium" v-model="pojo.teacher"></el-input>
        </el-form-item>
        <el-form-item label="工号">
          <el-input size="medium" v-model="pojo.job_num"></el-input>
        </el-form-item>
        <el-form-item label="联系方式">
          <el-input size="medium" v-model="pojo.contact_info"></el-input>
        </el-form-item>
    ...
    <el-dialog :visible.sync="dialogUploadVisible" title="批量导入" width="40%">
      <el-upload
        ...
        action="http://localhost:9999/demo/classes/importExcel/"
        class="upload-demo"
        ref="upload">
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        classesApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
          this.list = response.data.rows;
          this.total = response.data.total
        })
      },
      handleSave() {
        classesApi.update(this.id, this.pojo).then(response => {
          this.$message({
      ...
      handleEdit(id) {
        ...
        if (id !== '') { // 修改
          classesApi.findById(id).then(response => {
            if (response.flag) {
      ...
      handleDelete(id) {
        this.$confirm('确定要删除此纪录吗?', '提示', {
          ...
          classesApi.deleteById(id).then(response => {
