  ...
  <div>
    ...
    <el-form :inline="true">
      <el-form-item label="学号">
        <el-input placeholder="学号" size="small" v-model="searchMap.student_num"></el-input>
      </el-form-item>
      <el-form-item label="课程编号">
        <el-input placeholder="课程编号" size="small" v-model="searchMap.course_num"></el-input>
      </el-form-item>
    ...
    <el-table
      ...
      <el-table-column label="课程编号" prop="course_num"></el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="编辑" width="40%">
      <el-form label-width="100px" style="margin-right: 30px">
        <el-form-item label="学号">
          <el-input size="medium" v-model="pojo.student_num"></el-input>
        </el-form-item>
        <el-form-item label="课程编号">
          <el-input size="medium" v-model="pojo.course_num"></el-input>
        </el-form-item>
        <el-form-item label="成绩">
          <el-input size="medium" v-model="pojo.score"></el-input>
        </el-form-item>
    ...
    <el-dialog :visible.sync="dialogUploadVisible" title="批量导入" width="40%">
      <el-upload
        ...
        action="http://localhost:9999/demo/score/importExcel/"
        class="upload-demo"
        ref="upload">
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        scoreApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
          this.list = response.data.rows;
          this.total = response.data.total
        })
      },
      handleSave() {
        scoreApi.update(this.id, this.pojo).then(response => {
          this.$message({
      ...
      handleEdit(id) {
        ...
        if (id !== '') { // 修改
          scoreApi.findById(id).then(response => {
            if (response.flag) {
      ...
      handleDelete(id) {
        this.$confirm('确定要删除此纪录吗?', '提示', {
          ...
          scoreApi.deleteById(id).then(response => {
